...
{
    internal sealed class FragmentShader: ShaderBase // A fragment shader wrapper
    {
        private int _handle = int.MinValue;

        public FragmentShader(ContextBase context, string source)
            : base(context, source)
        {
        }
        ...
        {
            if (Compiled) // If we're compiled already, don't do it again
            ...
            _handle = Context.GetNewFragmentShaderObject(true);
