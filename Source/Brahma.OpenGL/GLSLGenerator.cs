...
namespace Brahma.OpenGL
{
    ...
    internal sealed class GLSLGenerator: ExpressionVisitor
    {
        ...
        // TODO: Add anonymous delegates that transform more method calls here. We to add more method support, preferably everything GLSL supports
        ...
        private static void IndexInto1D(GLSLGenerator sender, MethodCallExpression methodCall)
        {
            ...
            // Add the sampling correction. Remember, texels are points, not squares like pixels. A texel lies at the center of a pixel.
        ...
        private string Generate()
        {
            ...
            {
                if (memberAccess.IsOutputCoordAccess()) // Ignore any "output.Current..." accesses, those are allowed
                    continue;
            ...
            // The inverse dimensions of each texture. Used for re-normalization of texture co-ordinates
        ...
        {
            // Check to see if its a case where we're trying to access output.Current
            if ((m.Member.DeclaringType.IsOutput()) &&
                (m.Member.Name == "Current") &&
                (m.Expression == null))
            {
                // De-normalize the texture coordinate. It will be normalized just before tex2D'ing
                // The "(int)" is here because gl_TexCoord[0].x points to the center of the texel but don't want that
                _code.Append("((int)(gl_TexCoord[0].x * _brahma_minWidth))"); // We can afford to use only one component, because we know this is a DataParallelArray
                return m;
            }

            // Check to see if its a case where we're trying to access CurrentX or CurrentY from output
            if ((m.Member.DeclaringType.IsOutput()) &&
                (m.Expression == null))
            {
                ...
                {
                    case "CurrentX":
                        _code.Append("((int)(gl_TexCoord[0].x * _brahma_minWidth))"); // We know this is CurrentX of a DataParallArray2D
                        return m;

                    case "CurrentY":
                        _code.Append("((int)(gl_TexCoord[0].y * _brahma_minHeight))"); // We know this is CurrentY of a DataParallArray2D
                        return m;
                }
            }
